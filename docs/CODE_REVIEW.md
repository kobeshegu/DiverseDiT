...
- The recomposer has no direct timestep input.
...
1. **The decomposition is not identifiable from recomposition alone.** A degenerate solution can let Evolving carry the full target while Persistent is invariant but ignored. The current variance/decorrelation options and branch-ablation diagnostics can expose or discourage this, but they do not constitute a proof. Treat “Persistent learns semantics” as a hypothesis until zero-replacement, capacity-sweep, branch-usage, and linear-probe evidence pass; if it fails, add a Persistent-only common-target objective or an explicit branch-usage bottleneck before a full run.
...
4. **Representation probes are specified but not yet automated.** Training logs provide online health checks; class/timestep/noise probes, CKA, gradient cosine, and counterfactual swaps still require an analysis runner.
...
```bash
pytest -q tests/test_trajectory_factorization.py
python -m py_compile models/sit.py loss.py train.py generate.py train_t2i.py
bash -n scripts/tfcr_ablation.sh
bash -n scripts/train_tfcr.sh
```

Then run a 100-step smoke job with `SiT-B/2`, local batch >= 2, `factor_batch_ratio=0.5`, and `report_to=none`. Confirm all logged values are finite and both projector/recomposer gradients are non-zero.
